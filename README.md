...
Definição atual: na Meta Ads, visualizações da página de destino (tratadas no painel como visitantes do site) / vendas reportadas × 100. No Google Ads, usa-se a taxa nativa `conversions_from_interactions_rate`. No consolidado, usa-se a média simples entre as taxas da Meta Ads e do Google Ads. As duas plataformas não têm a mesma base de cálculo e não representam pessoas únicas.
