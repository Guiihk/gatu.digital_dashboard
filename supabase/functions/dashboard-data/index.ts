...
Deno.serve(async req=>{
  ...
  try{
    ...
    const {data:metricRows,error:metricError}=campaignIds.length?await adminClient.from('daily_campaign_metrics').select('campaign_id,metric_date,spend,impressions,clicks,conversions,conversion_value').in('campaign_id',campaignIds).gte('metric_date',previousStartDate).lte('metric_date',endDate):{data:[],error:null};
    if(metricError) throw metricError;
    const totals=new Map<string,{spend:number,impressions:number,clicks:number,conversions:number,conversionValue:number}>();
    for(const row of (metricRows||[]).filter(row=>row.metric_date>=startDate)){
      const current=totals.get(row.campaign_id)||{spend:0,impressions:0,clicks:0,conversions:0,conversionValue:0};
      current.spend+=Number(row.spend||0);current.impressions+=Number(row.impressions||0);current.clicks+=Number(row.clicks||0);current.conversions+=Number(row.conversions||0);current.conversionValue+=Number(row.conversion_value||0);
      totals.set(row.campaign_id,current);
    }
    const campaigns=(campaignRows||[]).map(row=>({id:row.id,externalId:row.external_id,platform:accountPlatform.get(row.ad_account_id)==='meta_ads'?'meta':'google',name:row.name,type:row.objective||'Campanha',status:row.status||'UNKNOWN',...(totals.get(row.id)||{spend:0,impressions:0,clicks:0,conversions:0,conversionValue:0})})).filter(row=>row.spend||row.impressions||row.clicks||row.conversions||row.conversionValue);
    const previousTotals=new Map<string,{spend:number,impressions:number,clicks:number,conversions:number,conversionValue:number}>();
    for(const row of (metricRows||[]).filter(row=>row.metric_date>=previousStartDate&&row.metric_date<=previousEndDate)){const current=previousTotals.get(row.campaign_id)||{spend:0,impressions:0,clicks:0,conversions:0,conversionValue:0};current.spend+=Number(row.spend||0);current.impressions+=Number(row.impressions||0);current.clicks+=Number(row.clicks||0);current.conversions+=Number(row.conversions||0);current.conversionValue+=Number(row.conversion_value||0);previousTotals.set(row.campaign_id,current)}
    const previousCampaigns=(campaignRows||[]).map(row=>({id:row.id,platformId:row.external_id,platform:accountPlatform.get(row.ad_account_id)==='meta_ads'?'meta':'google',name:row.name,type:row.objective||'Campanha',status:row.status||'UNKNOWN',...(previousTotals.get(row.id)||{spend:0,impressions:0,clicks:0,conversions:0,conversionValue:0})}));
    const campaignPlatform=new Map(campaigns.map(row=>[row.id,row.platform]));
    const daily=(metricRows||[]).filter(row=>row.metric_date>=startDate).map(row=>({campaignId:row.campaign_id,platform:campaignPlatform.get(row.campaign_id),date:row.metric_date,spend:Number(row.spend||0),impressions:Number(row.impressions||0),clicks:Number(row.clicks||0),conversions:Number(row.conversions||0),conversionValue:Number(row.conversion_value||0)})).filter(row=>row.platform);
