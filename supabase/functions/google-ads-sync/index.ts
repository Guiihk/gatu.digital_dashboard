...
async function synchronize(clientId:string,startDate:string,endDate:string){
  ...
  try{
    ...
    const [rows,conversionRows]=await Promise.all([
      googleSearch(customerId,managerId,token,`SELECT campaign.id, campaign.name, campaign.status, campaign.advertising_channel_type, segments.date, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions, metrics.conversions_value, metrics.conversions_from_interactions_rate FROM campaign WHERE segments.date BETWEEN '${startDate}' AND '${endDate}' AND campaign.status != 'REMOVED' ORDER BY segments.date`),
      googleSearch(customerId,managerId,token,`SELECT campaign.id, segments.date, segments.conversion_action_category, metrics.conversions, metrics.conversions_value FROM campaign WHERE segments.date BETWEEN '${startDate}' AND '${endDate}' AND campaign.status != 'REMOVED' ORDER BY segments.date`)
    ]);
    ...
    const leadCategories=new Set(['LEAD','CONTACT','PHONE_CALL_LEAD','SUBMIT_LEAD_FORM','BOOK_APPOINTMENT','REQUEST_QUOTE','QUALIFIED_LEAD','CONVERTED_LEAD','IMPORTED_LEAD']);
    const salesByCampaignDate=new Map<string,number>(),leadsByCampaignDate=new Map<string,number>(),salesValueByCampaignDate=new Map<string,number>();
    conversionRows.forEach(row=>{const campaign=row.campaign as Record<string,unknown>,segments=row.segments as Record<string,unknown>,values=row.metrics as Record<string,unknown>;const key=`${campaign?.id}:${segments?.date}`;const category=String(segments?.conversionActionCategory||segments?.conversion_action_category||'');const conversions=Number(values.conversions||0),value=Number(values.conversionsValue||values.conversions_value||0);if(['PURCHASE','STORE_SALE','SUBSCRIBE_PAID'].includes(category)){salesByCampaignDate.set(key,(salesByCampaignDate.get(key)||0)+conversions);salesValueByCampaignDate.set(key,(salesValueByCampaignDate.get(key)||0)+value)}else if(leadCategories.has(category)){leadsByCampaignDate.set(key,(leadsByCampaignDate.get(key)||0)+conversions)}});
    const metrics=rows.map(row=>{const campaign=row.campaign as Record<string,unknown>,segments=row.segments as Record<string,unknown>,values=row.metrics as Record<string,unknown>,key=`${campaign?.id}:${segments?.date}`,reportedSales=salesByCampaignDate.get(key)||0,reportedLeads=leadsByCampaignDate.get(key)||0;return {campaign_id:localIds.get(String(campaign.id)),metric_date:String(segments.date),spend:Number(values.costMicros||values.cost_micros||0)/1_000_000,impressions:Number(values.impressions||0),clicks:Number(values.clicks||0),conversions:reportedSales+reportedLeads,reported_sales:reportedSales,reported_leads:reportedLeads,google_conversion_rate:Number(values.conversionsFromInteractionsRate??values.conversions_from_interactions_rate??0),conversion_value:salesValueByCampaignDate.get(key)||0,currency_code:String(account.currency_code||'BRL'),source_updated_at:new Date().toISOString(),updated_at:new Date().toISOString()}}).filter(row=>row.campaign_id);
