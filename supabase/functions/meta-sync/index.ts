...
function selectedAction(actions: unknown) {
  ...
  const priority = ['lead', 'onsite_conversion.messaging_conversation_started_7d', 'offsite_conversion.fb_pixel_lead', 'offsite_conversion.fb_pixel_purchase'];
  for (const type of priority) {
    const match = values.find(item => item.action_type === type);
    if (match) return {type, value: Number(match.value || 0)};
  }
...
async function synchronize(clientId: string, startDate: string, endDate: string) {
  ...
  try {
    ...
    const metricPayload = insightRows.map(row => {
      const conversion = selectedAction(row.actions);
      const value = selectedAction(row.action_values);
      return {
        ...
        conversion_value: value.type === conversion.type ? value.value : 0,
